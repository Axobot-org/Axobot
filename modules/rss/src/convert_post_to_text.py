...
async def _convert_post_to_text(post: str, post_type: str) -> str | None:
    "Convert an RSS feed post text into a plain text string"
    if post_type == "text/plain":
        return post
    if post_type == "text/html":
        return await _convert_html_to_text(post)
    if post_type == "application/xhtml+xml":
        return await _convert_xhtml_to_text(post)

...
class BlueskyRSS:
    ...
    async def _parse_entry(self, entry: FeedParserDict, feed: FeedParserDict, url: str, channel:"discord.abc.MessageableChannel"):
        ...
        if (author_match := re.search(r"^@[\w._-]+ - ([^\n]+)$", full_author)) is None:
            raise ValueError("Author name not found in feed title, cannot create RssMessage")
        author = author_match.group(1).strip()
        post_text = await get_text_from_entry(entry)
        if not post_text:
            raise ValueError("Post text is empty, cannot create RssMessage")
        return RssMessage(
            bot=self.bot,

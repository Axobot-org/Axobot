...
class AddCaseView(ui.View):
    """View to add a ban to the user cases"""
    ...
    async def _add_case_btn(self, interaction: discord.Interaction, _button: ui.Button[Self]):
        """Add the ban to the user cases on click"""
        # Only members with Ban Members permission can use it
        if not interaction.guild or not isinstance(interaction.user, discord.Member):
            return
        if not interaction.user.guild_permissions.ban_members:
            await interaction.response.send_message("You need the 'Ban Members' permission to do that.", ephemeral=True)
            return
        ...
        try:
            ...
            ok = await cases_cog.db_add_case(case)  # type: ignore[attr-defined]
            if ok:
    ...
    async def on_timeout(self):
        try:
            ...
            self.bot.dispatch("error", err, "AddBanCaseView timeout edit")

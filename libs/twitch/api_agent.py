...
from libs.twitch.api_types import HttpTokenNotSet, TokenResponse
...
class TwitchApiAgent:
    ...
    async def get_user_stream(self, *user_name: str):
        "Get the stream of a user"
        if not self.is_token_valid:
            raise HttpTokenNotSet()
        if not self._session:
            self._session = aiohttp.ClientSession()
        url = "https://api.twitch.tv/helix/streams?user_login=livetilla"
